    ...
    <div className="min-h-screen">
      <header className="sticky top-0 z-50 border-b bg-[#f7f7f7]/90 backdrop-blur">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 flex h-16 items-center justify-between">
          ...
          <Link href="/blog/feed.xml" className="flex items-center space-x-2">
      ...
      <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <h1 className="text-3xl font-bold mb-8 text-center">Yasuhisa Honda&apos;s Blog</h1>
        <div className="space-y-12">
          {articles.map((article) => (
            <article key={article.id} className="border-b border-gray-300 pb-8 last:border-b-0">
              <div className="flex flex-col sm:flex-row gap-6">
                ...
                <div className="sm:w-2/3 space-y-2">
                  ...
                  <Link href={`/blog/${article.id}`} className="no-underline space-y-1 block">
                    ...
                    <p className="text-sm text-gray-700">{excerpt(article.content)}</p>
                  </Link>
                </div>
              </div>
            </article>
