        ...
        <div className="flex flex-col sm:flex-row gap-8 p-3">
          ...
          <div className="sm:w-2/3 space-y-2">
            <div className="flex items-center gap-4 text-sm text-gray-300">
              ...
              <span className="px-4 py-1 bg-gray-800/30  text-xs rounded-full">
                {readTime(article.content)}
              </span>
              {article.tags.map((tag) => (
                <span key={tag} className="px-2 py-1 bg-[#222222] text-xs rounded">
                  {tag}
                </span>
              ))}
            </div>
  ...
  return (
    ...
    >
      <div className=" w-full p-4 mb-8 bg-gradient-to-r from-white-400 via-black-500 to-white-600 rounded-lg flex animate-gradient-x underline hover:from-blue-400 hover:via-black-500 hover:to-blue-600">
        <Image src="/zenn-logo-only.svg" alt="Zenn" width={24} height={24} className="h-6 w-6" />
