...
export function HTMLContent({ content }: HTMLContentProps) {
  ...
  const [amazonLinks, setAmazonLinks] = useState<
    Array<{ url: string; text: string; id: string; productInfo?: AmazonProductInfo }>
  >([]);
        ...
        if (productInfo) {
          setAmazonLinks((prevLinks) =>
            prevLinks.map((prevLink) =>
              prevLink.id === link.id ? { ...prevLink, productInfo } : prevLink,
            ),
          );
        }
      } catch (error) {
  ...
  const renderContent = () => {
    ...
    parts.forEach((part, index) => {
      ...
      if (amazonLinkMatch) {
        ...
        result.push(<div key={`content-${index}`} dangerouslySetInnerHTML={{ __html: part }} />);
      }
    });
